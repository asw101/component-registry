...
fn fallback_library_surface(bytes: &[u8]) -> Option<LibrarySurface> {
    ...
    fn prim_to_wit(p: PrimitiveValType) -> Option<WitTy> {
        match p {
            ...
            PrimitiveValType::ErrorContext => None,
        }
    }
    ...
    type InnerFuncTy = (Vec<(String, ComponentValType)>, Option<ComponentValType>);

    let mut depth: u32 = 0;
    let mut in_inner_component = false;
    let mut inner_types: Vec<Option<InnerFuncTy>> = Vec::new();
    ...
    for payload in Parser::new(0).parse_all(bytes) {
        ...
        match payload {
            ...
            Payload::ComponentExportSection(reader) if depth == 2 && in_inner_component => {
                for export in reader.into_iter().flatten() {
                    ...
                    for (pname, pty) in params {
                        if let Some(wty) = cval_to_wit(*pty) {
            ...
            Payload::CustomSection(cs) if depth == 1 && cs.name() == "package-docs" => {
                pkg_docs_json = std::str::from_utf8(cs.data()).ok().map(String::from);
            }
            _ => {}
        }
    }
            ...
            for (iname, ival) in ifaces {
                ...
                if let Some(funcs) = ival.get("funcs").and_then(|v| v.as_object()) {
                    for (fname, fval) in funcs {
                        let doc = fval.get("docs").and_then(|v| v.as_str()).map(String::from);
...
pub fn extract_library_surface(bytes: &[u8]) -> Result<LibrarySurface, LibraryExtractError> {
    ...
    for (key, item) in &world.exports {
        match item {
            WorldItem::Function(func) => match func_to_decl(&resolve, &func.name, func) {
                Ok(decl) => items.push(LibraryItem::Func(decl)),
                Err(e @ LibraryExtractError::Resource { .. }) => return Err(e),
                Err(_) => {} // skip funcs with unsupported types (streams, futures, etc.)
            },
